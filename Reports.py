...
if __name__ == '__main__':
    # naive_mitigation-pruning_cifar_simple_h1.0_c20_50
    # naive_mine_cifar_simple_h10000_c50_100
    report = load_report('./result-feddill1/ff_mine2_cifar_simple_h10000_c50_500')
    # report = load_report('./result-fedavg/naive_mitigation-pruning_cifar_simple_h1.0_c20_2')
    asr = []
    acc = []
    print(report.all_records)
    for i in report.all_records:
        print(type(i))
        for k, v in i.rounds.items():
            for k1, v1 in v.items():
                if k1 == 'asr':
                    asr.append(v1[0]['Top-1'])
                else:
                    acc.append(v1[0]['Top-1'])
    with open(r'D:\\houmengongji\\report\\res19.npy', 'ab') as f:  # D:\houmengongji\report
        np.save(f, asr)
        np.save(f, acc)
