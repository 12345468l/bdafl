...
class FederatedTask:
    ...
    def build_optimizer(self, model=None) -> optim.Optimizer:
        if model is None:
            model = self.model
        #if lambda layer: layer.requires_grad is None:
            #print("error!!@!")
    ...
    def set_input_shape(self):
        ...
        #print("-----------------------self.train_dataset.shape:", self.train_dataset[0][0])
        #print("-----------------------inp.shape:",inp.shape) #[3, 150, 250]
        self.params.input_shape = inp.shape

    def reset_metrics(self):  # 重置指标
        for metric in self.metrics:
            metric.reset_metric()

    def get_batch(self, batch_id, data) -> Batch:
        """Process data into a batch. 将数据处理成一个批次。

        这个方法针对不同的数据集和数据加载器，通过返回Batch类的对象，统一了输出结果。
        Specific for different datasets and data loaders this method unifies the output by returning the object of class Batch.
        :param batch_id: id of the batch 批次的id
        :param data: object returned by the Loader. 由加载器返回的对象
        :return:
        """
        inputs, labels = data
        #print("---------------------get_batch data:", len(data))  # inputs [4, 3, 150, 250] data 2
        batch = Batch(batch_id, inputs, labels)
        #print("---------batch:",batch.size())
        return batch.to(self.params.device)  # 返回一个新的Batch对象，其中包含了转移到cuda后的数据
    ...
    def sample_dirichlet_train_data(self, n_client):
        """
            ...
            输入：参与者数量和 alpha（分布的参数）
            输出：一个索引列表，表示 CIFAR 训练集中的数据。
            要求：cifar_classes，一个预处理过的类索引字典。
            示例方法：使用均匀采样的 10 维向量作为狄利克雷分布的参数，来采样每个类中图像的数量。
        """
        alpha = self.params.heterogenuity  # 异质性

        total_classes = dict()
        # 将数据集中相同标签的数据索引归类到一个字典 `total_classes` 中
        for ind, x in enumerate(self.train_dataset):
            _, label = x
            if label in total_classes:  # 如果x的标签已经存在，就将x的索引添加到该标签对应的列表中
                total_classes[label].append(ind)
            else:  # 否则就创建一个新的键值对，键为x的标签，值为包含x索引的列表
                total_classes[label] = [ind]

        #print("total_classes:::",len(total_classes.keys())) 2

        class_size = len(total_classes[0])  # 键为0对应的值的数量，即一个类别包含多少数据
        per_client_list = defaultdict(list)  # 创建了一个默认字典，值的类型是列表
        n_class = len(total_classes.keys())  # 键的数量，即数据有多少类别

        np.random.seed(111)
        for n in range(n_class):  # 对于每个类别
            random.shuffle(total_classes[n])  # 对该类别的数据随机打乱
            n_party = n_client  # 客户端数量
            if self.params.server_dataset:  # 如果参数中server_dataset为true，则为服务器分配一个额外的客户端
                sampled_probabilities = class_size * np.random.dirichlet(np.array(n_client * [alpha] + [alpha]))  # 是一个长度为 `n_client + 1` 的数组，表示了样本分布的概率。
                n_party = n_party + 1

            else:
                sampled_probabilities = class_size * np.random.dirichlet(np.array(n_client * [alpha]))
            for p in range(n_party):  # 对每一个客户端
                # 根据样本的概率分布，从每个类别中选择一定数量的样本，并将这些样本添加到对应客户端的列表中
                n_image = int(round(sampled_probabilities[p]))
                sampled_list = total_classes[n][:min(len(total_classes[n]), n_image)]  # 从类别 `n` 中选择最多 `n_image` 个样本（如果类别 `n` 中的样本数量少于 `n_image`，则选择全部样本）

                per_client_list[p].extend(sampled_list)  # 将被选择的样本添加到per_client_list中，其中存储了每个客户端的样本列表
                # decrease the chosen samples 移除已分配的数据
...
class Cifar10FederatedTask(FederatedTask):  # 父类是FederatedTask
    def __init__(self, params: Params):
        super(Cifar10FederatedTask, self).__init__(params)
        self.means = (0.4914, 0.4822, 0.4465)  # 均值
        self.lvars = (0.2023, 0.1994, 0.2010)  # 方差

        self.normalize = transforms.Normalize(self.means, self.lvars)  # 使用给定的均值和方差来标准化数据
    ...
    def build_model(self) -> Module:
        if self.params.model == 'resnet18':
            if self.params.pretrained:
                model = resnet18(pretrained=True)  # 在resnet.py中对模型进行初始化和设置
...
class financeDataset1(Dataset):
    ...
    def _create_class_idx_dict_test(self):
        val_image_dir = os.path.join(self.test_dir, "images")
        if sys.version_info >= (3, 5):
            images = [d.name for d in os.scandir(val_image_dir) if d.is_file()]
        else:
            images = [d for d in os.listdir(val_image_dir) if os.path.isfile(os.path.join(val_image_dir, d))]
        val_annotations_file = os.path.join(self.test_dir, "val_annotations.txt")
        self.val_img_to_class = {
        }
        set_of_classes = set()
        with open(val_annotations_file, 'r') as fo:
...
class financeFederatedTask(FederatedTask):
    def __init__(self, params: Params):
        super(financeFederatedTask, self).__init__(params)
        self.means = (0.98994124,0.9865057,0.98228145)
        self.lvars = (0.07752948,0.0918369,0.1119803)
        self.normalize = transforms.Normalize(self.means, self.lvars)
        self.data_dir = ''
    ...
    def load_finance_data(self):
        ...
        #self.train_dataset = financeDataset1(self.data_dir, train=True, transform=transform_train)
        #self.test_dataset = financeDataset1(self.data_dir, train=False, transform=transform_test)

        self.train_dataset = financeDataset(txt_path='', transform=transform_train)
        self.train_dataset = financeDataset(txt_path='', transform=transform_test)
        ...
        self.test_loader = DataLoader(self.test_dataset,
                                      batch_size=self.params.test_batch_size,
                                      shuffle=False, num_workers=0)
        self.classes = [i for i in range(2)]
...
if __name__ == "__main__":
    with open('configs/finance_fed.yaml') as f:
        params = yaml.load(f, Loader=yaml.FullLoader)

    params = Params(**params)
    task = financeFederatedTask(params)
