...
def distance_ranks(stacked_vectors):
    stacked_vectors = torch.stack(stacked_vectors)
...
class ServerAvg(Serverbase):
    ...
    def select_participated_clients(self, fixed_mal):  # 实际效果是在良性和恶意混合的50个客户端中随机选择两个
        n_chosen = int(self.chosen_rate * self.n_clients)  # 要选择的客户端数量
        candidate_ids = list()
        for c in range(self.n_clients):  # 遍历所有客户端，
            if not c in fixed_mal:  # 如果客户端不在fixed_mal中，（fixed_mal中大概存储了恶意客户端）
                candidate_ids.append(c)  # 将该客户端添加到candidate_ids中
        selected_benigns = np.random.choice(candidate_ids, n_chosen - len(fixed_mal), replace=False)  # 从良性客户端中随计选择
        return list(selected_benigns) + list(fixed_mal)  # 将良性客户端和恶意客户端合并成一个列表

    def broadcast_model_weights(self, clients: client_group):
        self.global_model.train()  # 将全局模型设置为训练模式
        for client in clients:  # 对于列表中的每一个客户
            client.set_model_weights(self.global_model)  # 根据全局模型权重更新本地模型
    ...
    def aggregate_global_model(self, clients: client_group, chosen_ids, pts):
        ...
        total_prop = 0
        if pts is None:  # 默认将每个被选中的客户端权重设置为1
            pts = [1 for i in range(len(chosen_ids))]

        # 对于每个被选中的客户端，根据其样本数量和权重，计算其对于全局模型的贡献
        ...
        self.global_model.load_state_dict(averaged_weights)  # 将计算得到的平均权重加载到全局模型中

    def collect_benign_client(self, task, clients: client_group, chosen_ids, pts):
        from sklearn.cluster import KMeans
        client_ranks = list()
        for id in chosen_ids:
            client = clients[id]
            client_ranks.append(client.get_conv_rank(task, n_test_batch=10, location='last').cpu())

        kmeans = KMeans(n_clusters=2, random_state=0).fit(client_ranks)
        #print(chosen_ids, kmeans.labels_)

        sum = np.sum(kmeans.labels_)
        print(sum)
        benign_ids = []
        if sum < len(chosen_ids) / 2:
            for i in range(len(chosen_ids)):
                if kmeans.labels_[i] == 0:
                    benign_ids.append(chosen_ids[i])
        else:
            for i in range(len(chosen_ids)):
    ...
    def collect_s_benign_client(self, task, clients: client_group, chosen_ids, pts):
        from sklearn.cluster import KMeans
        client_ranks = list()
        for id in chosen_ids:
            client = clients[id]
            client_ranks.append(client.get_filter_ranks().cpu())
        # client_ranks = distance_ranks(client_ranks)
        # client_ranks = torch.unsqueeze(torch.tensor(client_ranks), 1)
        kmeans = KMeans(n_clusters=2, random_state=0).fit(client_ranks)
        sum = np.sum(kmeans.labels_)
        print(sum)
        benign_ids = []
    ...
    def collect2_benign_client(self, task, clients: client_group, chosen_ids, pts):  # K-Means
        from sklearn.cluster import KMeans
        client_ranks = list()
        for id in chosen_ids:
            client = clients[id]
        ...
        # client_ranks = distance_ranks(client_ranks)
        # client_ranks = torch.unsqueeze(torch.tensor(client_ranks), 1)
        #print(type(client_ranks))
        #client_ranks_array = np.array(client_ranks)
        #client_ranks_cpu = client_ranks.to('cpu')
        print("client_ranks:",client_ranks)
        kmeans = KMeans(n_clusters=2, random_state=0).fit(client_ranks)
        ...
        #k = 0
        # for i in range(len(chosen_ids)):
        #     clients[i].his_cum += kmeans.labels_[i]
        # for id in chosen_ids:
        #     clients[id].his_cum += kmeans.labels_[k]
        #     k = k + 1

        if sum < len(chosen_ids) / 2:  # 哪个簇包含的客户数量多，假定该簇为良性
            for i in range(len(chosen_ids)):
                print("###################",i,chosen_ids[i],kmeans.labels_[i])
                if kmeans.labels_[i] == 0:
                    clients[chosen_ids[i]].his_cum = clients[chosen_ids[i]].his_cum + (1)
                    tem[i] = 1
                else:
                    clients[chosen_ids[i]].his_cum = clients[chosen_ids[i]].his_cum + (-1)
                    tem[i] = 0
        ...
        for i in range(len(chosen_ids)):
            print(chosen_ids[i], ":", clients[chosen_ids[i]].his_cum)
            if clients[chosen_ids[i]].his_cum > 0:
                benign_ids.append(chosen_ids[i])
            elif clients[chosen_ids[i]].his_cum == 0:
                if tem[i] == 1:
                    benign_ids.append(chosen_ids[i])
    ...
    def collect3_benign_client(self, task, clients: client_group, chosen_ids, pts):  # 孤立森林
        from sklearn.cluster import KMeans
        client_ranks = list()
        for id in chosen_ids:
            client = clients[id]
            client_ranks.append(client.get_filter_ranks())
        # client_ranks = distance_ranks(client_ranks)
        # client_ranks = torch.unsqueeze(torch.tensor(client_ranks), 1)
        ...
        # random_state=rng
        clf = IsolationForest()
        clf.fit(client_ranks)
        y_pred = clf.predict(client_ranks)
    ...
    def conv_pruning(self, task, orders):
        ...
        for i, conv_id in enumerate(orders):
            ...
            print("weight: ", model_weights[final_conv].shape)
