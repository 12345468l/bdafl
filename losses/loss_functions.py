...
def normal_loss(model, criterion, inputs, labels, grads):
    #print("---------------------inputs:",inputs.size()) #[4, 3, 150, 250]
...
def trigger_loss(model, backdoor_inputs, clean_inputs, pattern, grads=True):
    model.train()  # 将本地模型设置为训练模式
    backdoor_activations = model.first_activations(backdoor_inputs).mean([0, 1])  # 通过模型获取后门输入和干净输入的第一层激活，并计算它们的均值
    clean_activations = model.first_activations(clean_inputs).mean([0, 1])
    difference = backdoor_activations - clean_activations  # 计算后门输入和干净输入的激活之间的差异
    loss = torch.sum(difference * difference)  # 计算差异的平方和，作为损失值
